-- Update the calculate_prediction_points function to handle match resets
...
BEGIN
  ...
  IF OLD.status = 'finished' AND (NEW.status IS NULL OR NEW.status != 'finished') THEN
    -- Reverse points for all predictions on this match
    FOR pred IN 
      SELECT 
        p.id, 
        p.user_id, 
    ...
    LOOP
      ...
      -- Decrement profile stats (use GREATEST to avoid negative values)
      UPDATE profiles SET 
  ...
  IF NEW.status = 'finished' AND (OLD.status IS NULL OR OLD.status != 'finished') THEN
    ...
    -- Calculate points for each prediction on this match
    FOR pred IN 
      SELECT 
        p.id,
        p.user_id,
      ...
      WHERE p.match_id = NEW.id
    LOOP
      -- Get league-specific scoring settings (or defaults)
      IF pred.league_id IS NOT NULL THEN
        SELECT 
          COALESCE(l.exact_score_points, 5) as exact_score_points,
          COALESCE(l.outcome_points, 2) as outcome_points,
          l.stage_multipliers
        INTO league_settings
        FROM leagues l
        WHERE l.id = pred.league_id;
      ELSE
        -- Default settings for solo bracket
        league_settings := ROW(5, 2, '{"group": 1, "round_of_32": 1.5, "round_of_16": 2, "quarter_final": 2.5, "semi_final": 3, "third_place": 3, "final": 4}'::jsonb);
      END IF;

      exact_pts := COALESCE(league_settings.exact_score_points, 5);
      outcome_pts := COALESCE(league_settings.outcome_points, 2);
      
      -- Get stage multiplier
      stage_mult := COALESCE(
        (league_settings.stage_multipliers->>NEW.stage::text)::numeric,
        1
      );
      ...
      -- Check if correct outcome (win/draw/loss)
